...
class WpakComponentTypeMaps extends WpakComponentType {

	protected function compute_data( $component, $options, $args = array() ) {
		global $wpdb;

		do_action( 'wpak_before_component_wpak_maps', $component, $options );
		...
		/**
		 * Filter the number of posts displayed in the map component.
		 *
		 ...
		 */
		$query_args['posts_per_page'] = apply_filters('wpak_maps_posts_per_page', WpakSettings::get_setting( 'posts_per_page' ), $component, $options, $args );
		...
		/**
		 * Filter args of the WP query that retrieve posts displayed on maps
		 *
		 ...
		 */
		$query_args = apply_filters( 'wpak_maps_query_args', $query_args, $component, $options, $args, $query );
	...
	protected static function get_post_data( $component, $_post ) {
		...
		//Add maps related data :
		if ( function_exists( 'get_field' ) ) {
			//ACF position field :
			$position = get_field( 'position' );
			if( !empty($position) ) {
				$post_data['position'] = get_field( 'position' );
			}
		}

		/**
		 * Filter post data sent to the app from a WPAK Maps component.
		 *
		 * @param array 			$post_data    	The default post data sent to an app.
		 * @param WP_Post 			$post 			The post object.
		 * @param WpakComponent 	$component		The component object.
		 */
		$post_data = apply_filters( 'wpak_maps_post_data', $post_data, $post, $component );
	...
	public function echo_form_fields( $component ) {
		...
		<div class="component-params">
			...
			<select name="post-type" class="wpak-maps-post-type">
	...
	public function echo_form_javascript() {
		...
		<script type="text/javascript">
			(function() {
				var $ = jQuery;
				$('.wrap').delegate('.wpak-maps-post-type', 'change', function() {
					var post_type = $(this).find(":selected").val();
					WpakComponents.ajax_update_component_options(this, 'maps', 'change-post-list-option', {taxonomy: '', post_type: post_type});
				});
				$('.wrap').delegate('.wpak-maps-taxonomies', 'change', function() {
					var post_type = $(this).closest('.ajax-target').prev('div.component-params').find('select.wpak-maps-post-type').eq(0).find(":selected").val();
					var taxonomy = $(this).find(":selected").val();
					WpakComponents.ajax_update_component_options(this, 'maps', 'change-post-list-option', {taxonomy: taxonomy, post_type: post_type});
				});
			})();
		</script>
		<?php
	...
	protected static function echo_sub_options_html( $current_post_type, $current_taxonomy = '', $current_term = '', $current_hook = '' ) {
		...
		/**
		 * Filter taxonomies list displayed into a "Maps" component select field.
		 *
		 * @param array 	$taxonomies    	The default taxonomies list to display.
		 */
		$taxonomies = apply_filters( 'wpak_component_type_maps_form_taxonomies', $taxonomies );
	...
	public function get_options_from_posted_form( $data ) {
		...
		$options = array( 'post-type' => $post_type, 'taxonomy' => $taxonomy, 'term' => $term );
		return $options;
	}

}

WpakComponentsTypes::register_component_type( 'maps', array( 'label' => __( 'Maps', WpAppKit::i18n_domain ) ) );
